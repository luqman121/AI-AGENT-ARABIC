...
- Added a provider-neutral **Skills Runtime** inside `@wakil/skills` (the layer every model adapter
  already consumes) rather than a parallel package: strict-typed `AgentSkill` registry, a
  deterministic router (artifact type / MIME / extension / language / mode → minimal skill set), a
  loader, and a prompt compiler with token budgeting. The router loads only the relevant skills per
  request and never injects the whole catalog.
- Added the versioned customer-facing **core system prompt** (`core.ar.v1`) in version control (not
  a DB row), plus per-artifact developer-message addenda and platform safety rules. The compiler
  emits the same `{ system, developer, user }` shape the adapters use, with admin-only run metadata
  (prompt version, selected skill ids/versions, token estimate, locale, validation profile).
- Authored an original, internal runtime skill catalog (`website-design`, `arabic-rtl-ui`,
  `design-system-generator`, `premium-depth-shadow`, `design-critic`, `website-quality-gate`,
  `document-reader`, `pdf-studio`, `spreadsheet-studio`, `document-studio`, `presentation-studio`,
  `artifact-quality-gate`) with checksums, mirrored to `skills/<id>/SKILL.md` + `manifest.json` and
  a `skills/manifest.lock.json` generated by `scripts/generate-skill-catalog.ts`; a test blocks
  drift.
- Added security primitives: skill **trust levels** (only internal/reviewed/provider-managed may
  load or execute), untrusted-content **fencing** that neutralizes forged delimiters and defends
  against prompt injection from uploaded files, injection-signal detection, filename sanitization,
  path-traversal prevention, and an upload MIME/size allowlist.
- Documented sourcing and licensing in `docs/agent-skills-sources.md`: all runtime skills are
  original/`internal`; external references (UI/UX Pro Max, Anthropic frontend-design, Vercel agent
  skills, shadcn/ui, aegov-dls, Cairo, anthropics/skills) are recorded as reviewed references, not
  vendored or auto-installed. Added 39 unit tests (registry schema, routing scenarios, prompt
  composition/budgeting, trust gating, injection fencing, path traversal, catalog sync). Verified
